...
def resolve_client(
    ...
    them, so the tools and memory adapter work with nothing but the env var
    set. Self-hosted users override the URL. The API key is optional at
    construction time — a missing key only fails when a call is actually made.
    """
