...
type ApiDryRunExtractMemoriesRequest struct {
	ctx context.Context
	ApiService *MemoryAPIService
	bankId string
	dryRunExtractRequest *DryRunExtractRequest
	authorization *string
}

func (r ApiDryRunExtractMemoriesRequest) DryRunExtractRequest(dryRunExtractRequest DryRunExtractRequest) ApiDryRunExtractMemoriesRequest {
	r.dryRunExtractRequest = &dryRunExtractRequest
	return r
}

func (r ApiDryRunExtractMemoriesRequest) Authorization(authorization string) ApiDryRunExtractMemoriesRequest {
	r.authorization = &authorization
	return r
}

func (r ApiDryRunExtractMemoriesRequest) Execute() (*DryRunExtractionResult, *http.Response, error) {
	return r.ApiService.DryRunExtractMemoriesExecute(r)
}

/*
DryRunExtractMemories Dry-run fact extraction (preview, no persistence)
...
Preview what the retain step would extract from text WITHOUT changing the bank — no entity resolution, links, embeddings, or persistence. Returns the candidate facts and the LLM token usage. Every prompt-affecting setting (retain mission, extraction mode, chunk size, …) is overridable in the body to A/B a candidate config against the bank's current one. This is a read-only tool: nothing is stored.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param bankId
 @return ApiDryRunExtractMemoriesRequest
*/
func (a *MemoryAPIService) DryRunExtractMemories(ctx context.Context, bankId string) ApiDryRunExtractMemoriesRequest {
	return ApiDryRunExtractMemoriesRequest{
		ApiService: a,
		ctx: ctx,
		bankId: bankId,
...
func (a *MemoryAPIService) DryRunExtractMemoriesExecute(r ApiDryRunExtractMemoriesRequest) (*DryRunExtractionResult, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *DryRunExtractionResult
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MemoryAPIService.DryRunExtractMemories")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/default/banks/{bank_id}/memories/dry-run-extract"
	localVarPath = strings.Replace(localVarPath, "{"+"bank_id"+"}", url.PathEscape(parameterValueToString(r.bankId, "bankId")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.dryRunExtractRequest == nil {
		return localVarReturnValue, nil, reportError("dryRunExtractRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.dryRunExtractRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
...
type ApiListObservationScopesRequest struct {
	ctx context.Context
	ApiService *MemoryAPIService
	bankId string
	authorization *string
...
func (r ApiListObservationScopesRequest) Execute() (*ObservationScopesResponse, *http.Response, error) {
	return r.ApiService.ListObservationScopesExecute(r)
}
...
// Execute executes the request
//  @return ObservationScopesResponse
func (a *MemoryAPIService) ListObservationScopesExecute(r ApiListObservationScopesRequest) (*ObservationScopesResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ObservationScopesResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MemoryAPIService.ListObservationScopes")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/default/banks/{bank_id}/observations/scopes"
	localVarPath = strings.Replace(localVarPath, "{"+"bank_id"+"}", url.PathEscape(parameterValueToString(r.bankId, "bankId")), -1)
