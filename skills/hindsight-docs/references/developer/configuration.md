...
> **Entity labels** (`entity_labels`), **free-form entity extraction** (`entities_allow_free_form`), and **tag enumerations** (`tag_enumerations`) are configured per bank via the [bank config API](api/memory-banks.md#retain-configuration), not as global environment variables — each bank can have its own controlled vocabulary. See [Entity Labels](retain.md#entity-labels) and [tag_enumerations](api/memory-banks.md#tag-enumerations) for details.
