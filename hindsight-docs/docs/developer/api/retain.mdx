...
**Bank merge.** A bank can declare default `tag_enumerations` via the [bank config API](/developer/api/memory-banks#tag-enumerations). Per-retain entries are merged with the bank-level config by `namespace` — per-retain wins on collision, every other namespace from the bank config still applies. This lets a bank define a stable global vocabulary while individual retain calls can override or add axes for one-off content types.
