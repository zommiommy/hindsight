...
description: Five new integrations, retain integrity hardening, and smoother upgrades in Hindsight 0.7.2
...
Hindsight 0.7.2 is a reliability-and-reach release: five new framework integrations, a round of **retain integrity** fixes that stop memories from being silently lost, tougher consolidation under concurrency, and a smoother upgrade path onto the 0.7.x line. **All users on 0.7.x should upgrade**, especially anyone running consolidation at scale or upgrading an older PostgreSQL deployment.

<!-- truncate -->

- [**Five New Integrations**](#five-new-integrations): Google ADK, Flowise, Roo Code, Vapi, and Gemini Spark.
...
Hindsight's catalog grows with five new ways to give your agents persistent memory:

- **Google ADK** — connect Hindsight memory to agents built on Google's Agent Development Kit.
- **Flowise** — drop-in Recall, Reflect, and Retain tool nodes for your Flowise flows.
- **Roo Code** — memory for the Roo Code assistant via MCP, including rules support.
- **Vapi** — send voice-agent interactions into Hindsight through a webhook integration.
- **Gemini Spark** — wire up Gemini Spark via MCP.

See the [integrations catalog](/integrations) for setup guides.
